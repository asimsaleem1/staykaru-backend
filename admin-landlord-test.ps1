...
Test-Endpoint "Admin Dashboard" {
    Invoke-RestMethod -Uri "$baseUrl/analytics/dashboard" -Method GET -Headers $adminHeaders | Out-Null
}

# 3. Get Pending Accommodations
Test-Endpoint "Pending Accommodations" {
    Invoke-RestMethod -Uri "$baseUrl/accommodations/admin/pending" -Method GET -Headers $adminHeaders | Out-Null
}

# 4. Get All Users
Test-Endpoint "All Users" {
    Invoke-RestMethod -Uri "$baseUrl/users/admin/all" -Method GET -Headers $adminHeaders | Out-Null
}

# 5. Get Food Providers
Test-Endpoint "Food Providers" {
    Invoke-RestMethod -Uri "$baseUrl/food-providers/admin/all" -Method GET -Headers $adminHeaders | Out-Null
}
...
# 1. Register Landlord
$landlordEmail = "landlord.test.$(Get-Random)@test.com"
$landlordPassword = "Test123!@#"
...
Test-Endpoint "Create Accommodation" {
    ...
    $response = Invoke-RestMethod -Uri "$baseUrl/accommodations" -Method POST -Body ($accommodationData | ConvertTo-Json) -Headers $landlordHeaders
...
Test-Endpoint "Landlord Dashboard" {
    Invoke-RestMethod -Uri "$baseUrl/accommodations/landlord/dashboard" -Method GET -Headers $landlordHeaders | Out-Null
}

# 6. Get Landlord Accommodations
Test-Endpoint "Landlord Accommodations" {
    Invoke-RestMethod -Uri "$baseUrl/accommodations/landlord" -Method GET -Headers $landlordHeaders | Out-Null
}

# 7. Get Landlord Profile
Test-Endpoint "Landlord Profile" {
    Invoke-RestMethod -Uri "$baseUrl/users/landlord/profile" -Method GET -Headers $landlordHeaders | Out-Null
}

# 8. Get Landlord Statistics
Test-Endpoint "Landlord Statistics" {
    Invoke-RestMethod -Uri "$baseUrl/users/landlord/statistics" -Method GET -Headers $landlordHeaders | Out-Null
}

# 9. Change Password
Test-Endpoint "Change Password" {
    $changePasswordData = @{
        oldPassword = $landlordPassword
        newPassword = "NewTest123!@#"
    }
    
    Invoke-RestMethod -Uri "$baseUrl/users/change-password" -Method PUT -Body ($changePasswordData | ConvertTo-Json) -Headers $landlordHeaders | Out-Null
}
