...
Write-Host "`n📡 1. BACKEND CONNECTIVITY TESTS" -ForegroundColor Magenta
...
Write-Host "`n👤 2. USER REGISTRATION TESTS" -ForegroundColor Magenta
...
# Test 3: Login Tests
Write-Host "`n🔑 3. USER LOGIN TESTS" -ForegroundColor Magenta
...
Write-Host "`n📱 4. SOCIAL LOGIN TESTS" -ForegroundColor Magenta
...
Write-Host "`n5. PROTECTED ENDPOINT TESTS" -ForegroundColor Magenta

# Test 5a: Access Protected Endpoint Without Token (Should Fail)
$protectedResult = Invoke-ApiRequest -Url "$BaseUrl/users/profile"
Log-TestResult -TestName "Protected Endpoint Without Token" -Success (!$protectedResult.Success) -Details "Should require authentication" -StatusCode $protectedResult.StatusCode

# Test 5b: Access Protected Endpoint With Valid Token (If we have one)
if ($studentLogin.Success -and $studentLogin.Data.accessToken) {
    $authHeaders = @{
        "Content-Type" = "application/json"
        "Authorization" = "Bearer $($studentLogin.Data.accessToken)"
    }
    
    $authenticatedResult = Invoke-ApiRequest -Url "$BaseUrl/users/profile" -Headers $authHeaders
    Log-TestResult -TestName "Protected Endpoint With Valid Token" -Success $authenticatedResult.Success -Details "Should allow access with valid JWT" -StatusCode $authenticatedResult.StatusCode
}

# Test 6: Password Reset Flow
Write-Host "`n🔄 6. PASSWORD RESET TESTS" -ForegroundColor Magenta

# Test 6a: Forgot Password Request
$forgotPasswordData = @{
    email = $studentData.email
}

$forgotResult = Invoke-ApiRequest -Url "$BaseUrl/auth/forgot-password" -Method "POST" -Body $forgotPasswordData
Log-TestResult -TestName "Forgot Password Request" -Success $forgotResult.Success -Details "Password reset email request" -StatusCode $forgotResult.StatusCode

# Test 7: Input Validation Tests
Write-Host "`n✅ 7. INPUT VALIDATION TESTS" -ForegroundColor Magenta

# Test 7a: Registration with Invalid Email (Should Fail)
$invalidEmailData = @{
    name = "Test User"
...
$invalidEmailResult = Invoke-ApiRequest -Url "$BaseUrl/auth/register" -Method "POST" -Body $invalidEmailData
Log-TestResult -TestName "Invalid Email Format" -Success (!$invalidEmailResult.Success) -Details "Should validate email format" -StatusCode $invalidEmailResult.StatusCode

# Test 7b: Registration with Weak Password (Should Fail)
$weakPasswordData = @{
    name = "Test User"
...
$weakPasswordResult = Invoke-ApiRequest -Url "$BaseUrl/auth/register" -Method "POST" -Body $weakPasswordData
Log-TestResult -TestName "Weak Password Validation" -Success (!$weakPasswordResult.Success) -Details "Should enforce password strength" -StatusCode $weakPasswordResult.StatusCode

# Test 8: Database Connectivity
Write-Host "`n🗄️ 8. DATABASE CONNECTIVITY TESTS" -ForegroundColor Magenta

# Test 8a: Get All Users (Public endpoint)
...
- Password validation rules may need adjustment
- Social login requires real OAuth tokens (as expected)
- Some endpoints require authentication (working as designed)
...
Write-Host "`n🎉 Authentication testing completed successfully!" -ForegroundColor Cyan
