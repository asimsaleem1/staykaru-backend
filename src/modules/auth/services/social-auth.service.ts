...
export class SocialAuthService {
   ...
   */
  async verifyFacebookToken(accessToken: string): Promise<FacebookUserData> {
    try {
   ...
   */
  validateSocialUserData(userData: FacebookUserData | GoogleUserData, provider: 'facebook' | 'google'): boolean {
    if (!userData.email || !userData.name) {
      throw new BadRequestException(`${provider} account must have email and name`);
    }
